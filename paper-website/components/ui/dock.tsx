...
function DockLabel({ children, className, ...rest }: DockLabelProps & { isHovered?: MotionValue<number> }) {
...
function DockIcon({ children, className, ...rest }: DockIconProps & { width?: MotionValue<number> }) {
  const { width } = rest;

  const defaultWidth = useMotionValue(20);
